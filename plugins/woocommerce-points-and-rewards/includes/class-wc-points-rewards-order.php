...
class WC_Points_Rewards_Order {
	...
	public function add_points_earned( $order ) {
		...
		update_post_meta( $order_id, '_wc_points_earned', $points/4 );
		...
		$order->add_order_note( sprintf( __( 'Customer earned %1$d %2$s for purchase.', 'woocommerce-points-and-rewards' ), $points/4, $wc_points_rewards->get_points_label( $points ) ) );
	}
