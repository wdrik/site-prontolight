...
//		WC()->session->set( 'wc_points_rewards_discount_amount', ( ! empty( $_POST['discount_amount'] ) ? absint( $_POST['discount_amount'] ) : '' ) );
		WC()->session->set( 'wc_points_rewards_discount_amount', ( ! empty( $_POST['discount_amount'] ) ?  number_format( $_POST['discount_amount'],2,'.','') : '' ) );
	...
	public function render_earn_points_message() {
		...
		$points_earned = $this->get_points_earned_for_purchase()/4;
		...
		// points earned
		$message = str_replace( '{points}', number_format( $points_earned,2,',','.' ), $message );
	...
	public function render_thank_you_message( $order_id ) {
		...
		$message = str_replace( '{points}', ( $points/4 ), $message );
		$message = str_replace( '{points_label}', $wc_points_rewards->get_points_label( $points/4 ), $message );

		$message = str_replace( '{total_points}', ( $total_points/4 ), $message );
		$message = str_replace( '{total_points_label}', $wc_points_rewards->get_points_label( $total_points/4 ), $message );

		$message = '<p>' . $message . '</p>';

		echo apply_filters( 'wc_points_rewards_thank_you_message', $message, $points/4, $total_points );
	}
...
//		$message .= '<input type="submit" class="button wc_points_rewards_apply_discount" name="wc_points_rewards_apply_discount" value="' . __( 'Apply Discount', 'woocommerce-points-and-rewards' ) . '" /></form>';
		...
		$message .= '<input type="submit" class="button wc_points_rewards_apply_discount" name="wc_points_rewards_apply_discount" value="Aplicar Desconto" /></form>';
		// wrap with info div
			...
			$(function() {
				...
				var get_order_total = function get_order_total() {
					...
					var order_total_clean = order_total_parts.filter( function( item ) {
						return ( ! isNaN( parseFloat( item, 10 ) ) || "." === item );
					});
		...
		if ( 'yes' === get_option( 'wc_points_rewards_partial_redemption_enabled' ) ) {
			...
			wc_enqueue_js(
				...
				$( "body" ).on( "click", "input.wc_points_rewards_apply_discount", function( e ) {
					var max_points = $( "[name=wc-points-rewards-max-points]" ).val()/4;
					var points = prompt( "Quantos Reais (R$) em desconto você gostaria de aplicar? ", max_points );
	...
	public static function get_discount_for_redeeming_points( $applying = false, $existing_discount_amounts = null, $for_display = false, $code = null ) {
		...
		if ( $applying && 'yes' === get_option( 'wc_points_rewards_partial_redemption_enabled' ) && WC()->session->get( 'wc_points_rewards_discount_amount' ) ) {
			...
			if ( $requested_user_discount > 0 && $requested_user_discount < $available_user_discount ) {
				$available_user_discount = $requested_user_discount*4;
			}
		}
